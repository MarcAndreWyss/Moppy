...
public class MoppyBridge {
    ...
    public MoppyBridge(String portName) throws NoSuchPortException, PortInUseException, UnsupportedCommOperationException, IOException {
        ...
        if(cpi.getPortType() == CommPortIdentifier.PORT_SERIAL){
            ...
            com.setSerialPortParams(Arduino.SERIAL_DATA_TRANSMISSION_RATE, SerialPort.DATABITS_8, SerialPort.STOPBITS_1, SerialPort.PARITY_NONE);
    ...
    public void sendEvent(byte pin, int periodData){
        sendArray(new byte[] {pin, (byte)((periodData >> 8) & 0xFF), (byte)(periodData & 0xFF)});
    }
    ...
    private void silenceDrives(){
        // Stop notes
        for (int d = Arduino.FIRST_PIN; d <= Arduino.MAX_PIN; d+=2){
